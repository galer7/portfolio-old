...
Deployments were done on Vercel's edge network and Fly.io's Dockerfile builder. Two deployment services were necessary because of Vercel's serverless functions having a maximum execution timeout, so they can't maintain a WS connection.
...
The last feature I've added was the CSS blurring effect while a food item's image is loading. This works using multiple `useState` variables for each user-selected food item.
